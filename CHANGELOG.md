...
- Naming-convention vocabulary: `sprint` domain (sprint and session management) and `start` action (initialises a session/branch from a clean baseline); `pm` domain (project management) and `manager` action (orchestrates an ongoing process without producing code).
